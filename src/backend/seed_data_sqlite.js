...
async function seedDatabase() {
    try {
        ...
        await db.run('DELETE FROM affiliates');
        ...
        for (const payment of payments) {
            const result = await db.run(
                'INSERT INTO payments (victim, amount, paid, campaign_id, paid_at, address) VALUES (?, ?, ?, ?, ?, ?)',
                [payment.victim, payment.amount, payment.paid, payment.campaign_id, payment.paid ? new Date().toISOString() : null, `FAKE-CRYPTO-ADDRESS-${Math.random().toString(36).substr(2, 9)}`]
            );
        }
        ...
        for (let i = 0; i < 25; i++) {
            ...
            const randomDate = new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(); // Random date within last 7 days
